...
namespace libc.base62 {
    public static class Base62 {
            ...
            while (true) {
                ...
                if (length == 6) // Not reaching the end
                {
                    if (read[0] >> 3 == 0x1f) // First 5-bit is 11111
                    {
                        ...
                        stream.Seek(-1,
                            SeekOrigin
                                .Current); // Leave the 6th bit to next group
        ...
        public static byte[] FromBase62(string base62) {
            ...
            foreach (var c in base62) {
                ...
                if (count == base62.Length - 1) {
                    ...
                    if (index >> (8 - mod) > 0)
                        throw new InvalidDataException(
                            "invalid ending character was found");
                    stream.Write(new[] {
                            (byte) (index << mod)
                        }, 0, 8 - mod);
                } else {
        ...
        public static string Encode62(string s) {
            return ToBase62(Encoding.UTF8.GetBytes(s));
        }
        public static string Decode62(string s) {
            return Encoding.UTF8.GetString(FromBase62(s));
        }
    }
}